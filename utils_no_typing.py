...
def load_data(dates = DATES, path = PATH):
    ...
    for file in exp_data:
        ...
        if int(date.split('-')[0]) < 9:
            num, valid = get_patient_num_old(os.path.join(path, file))
        else:
            num, valid = get_patient_num_new(os.path.join(path, file))
...
def fit_poly(y, t, num_par = 3):
    ...
    dt = (t.max() - t.min()) / 2
    t = (t - dt) / dt
    params_logistic, _ = curve_fit(_htan_time, t, y, p0=p0_logistic,
                                    maxfev=10000)#, bounds=(min_bounds,  max_bounds)

    R0, Rmax, t50 = params_logistic

    t_fit = np.linspace(min(t), max(t))
    R_logistic_fit = _htan_time(t_fit, R0, Rmax, t50)
...
def featurize_series(data, ind, method = 'logfit_2', aug_factor = 1):
        ...
        if method.startswith('logfit'):
            ...
            try:
                ...
                _, _, line1_par, _ = fit_logistic(data, t1, num_par)
                result = line1_par
            ...
            try:
                t1 = np.arange(len(data))
                _, _, line1_par, _ = fit_poly(data, t1, num_par)
                result = line1_par
